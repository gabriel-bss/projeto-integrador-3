...
namespace AtestadoMedico.Controllers
{
    ...
    public class AtestadoController : ControllerBase
    {
        ...
        public AtestadoController(ApplicationDbContext context, IConfiguration configuration, IWebHostEnvironment environment, IHttpClientFactory httpClientFactory)
        {
            _context = context;
        ...
        public async Task<IActionResult> Upload([FromForm] AtestadoViewModel viewModel)
        {
            ...
            // --- 1. LÓGICA DE UPLOAD (LOCAL: salva em wwwroot/uploads/) ---
        ...
        public async Task<IActionResult> Download(int id, [FromQuery] int usuarioId)
        {
            ...
            // LOCAL: Servir arquivo diretamente do sistema de arquivos
            if (atestado.CaminhoArquivo.StartsWith("http"))
            {
                return Redirect(atestado.CaminhoArquivo);
            }

            var localPath = atestado.CaminhoArquivo.TrimStart('/').Replace('/', Path.DirectorySeparatorChar);
            var filePath = Path.Combine(_environment.WebRootPath, localPath);

            if (!System.IO.File.Exists(filePath))
            {
                return NotFound("Arquivo do atestado não encontrado no servidor local.");
            }
        ...
        public async Task<IActionResult> ValidarCid([FromQuery] string codigo)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Console.WriteLine($"Validando CID '{codigo}'. Resposta da API: {jsonResponse}");
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, new { valido = false, mensagem = "Erro interno no servidor ao tentar validar o CID.", detalhe = ex.Message });
            }
        }
